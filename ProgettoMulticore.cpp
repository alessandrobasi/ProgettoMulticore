...
*/
bool _check_row(sudo sudoku, int trynum, int riga) {
    bool test = false;
    int incr;
...
bool _check_col(sudo sudoku, int trynum, int col) {
    bool test = false;
    int incr;
    for (incr = 0; incr < 9; incr++) {
...
bool _check_square(sudo sudoku, int trynum, int riga, int col) {
    riga /= 3;
    col /= 3;
    bool test = false;
...
*/
bool check_space(sudo sudoku, int trynum, int row, int col) {
...
void markup(sudo sudoku, int casella, sudonote &note) {
    ...
    for (int trynum = 1; trynum <= 9; trynum++) {
        ...
        if (check_space(sudoku, trynum, riga, colonna)) {
...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    //MPI_Gather(&sudoku, lavoro_per_thread, MPI_INT, &recv_sudoku, lavoro_per_thread, MPI_INT, ROOT, MPI_COMM_WORLD);

    if (rank == ROOT) {
        _pprint(recv_sudoku);
        sudoku = recv_sudoku;
    }
    // tutti i thread hanno un sudoku risolto dai singleton
    MPI_Barrier(MPI_COMM_WORLD);

    

    std::copy(note.data()->begin() + inizio, note.data()->begin() + fine, note.data()->begin());

    // A questo punto i thread devono scambiarsi le proprie tabelle con solo le caselle lavorate
    MPI_Allgather(&note, lavoro_per_thread, MPI_INT, &recv_note, lavoro_per_thread, MPI_INT, MPI_COMM_WORLD);
    //MPI_Gather(&sudoku, lavoro_per_thread, MPI_INT, &recv_sudoku, lavoro_per_thread, MPI_INT, ROOT, MPI_COMM_WORLD);
